 ...
 * `/api/*`. In dev, Vite
 * proxies `/api` here (see vite.config.ts), so the browser makes same-origin calls
 * and never sees the keys. The live WebSocket stays frontend-direct for now
 * (ADR-015); order placement is server-side only (ADR-018).
 */
